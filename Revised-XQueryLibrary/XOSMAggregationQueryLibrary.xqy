...
{
 let $list := xosm_ag:metricDistance($document,$street)
 return 
  fn:sort($list,function($oneway){xosm_kw:getTagValue($oneway,"distance")})[1] 
};

declare function xosm_ag:maxDistance($document as node()*, $street as node())
{
 let $list := xosm_ag:metricDistance($document,$street)
 return 
  fn:sort($list, function($oneway){-xosm_kw:getTagValue($oneway,"distance")})[1]
...
declare function xosm_ag:metricDistance($document as node()*, $street as node())
{
  for $each in $document
  let $distance := geo:distance(if ($each[@type="way"]) then xosm_gml:_osm2GmlLine($each)
  else if ($each[@type="area"]) then xosm_gml:_osm2GmlPolygon($each) 
  else xosm_gml:_osm2GmlPoint($each/node/@lat,$each/node/@lon),
  if ($street[@type="way"]) then xosm_gml:_osm2GmlLine($street) else 
  if ($street[@type="area"]) then xosm_gml:_osm2GmlPolygon($street) else
  xosm_gml:_osm2GmlPoint($street/node/@lat,$street/node/@lon)),
      $name := distinct-values($each//tag[@k='name']/@v)
  return 
    <oneway name = "{$name}" distance = "{$distance}">
    { $each/*}
  </oneway>
...
declare function xosm_ag:avgDistance($document as node()*, $street as node())
{
  let $list := xosm_ag:metricDistance($document,$street) (: CHANGE $list :)
  return 
   fn:avg(fn:for-each($list,
        function($oneway){xosm_kw:getTagValue($oneway,"distance")}))    
};

declare function xosm_ag:metricBottomCount($document as node()*, $metricOperator as xs:string, $k as xs:integer)
{
  let $list := xosm_ag:metricList($document,$metricOperator)
...
declare function xosm_ag:metricTopCount($document as node()*, $metricOperator as xs:string, $k as xs:integer)
{
  let $list := xosm_ag:metricList($document,$metricOperator)
  return fn:subsequence(fn:sort($list, 
         function($oneway){-xosm_kw:getTagValue($oneway,$metricOperator)}),1,$k)
...
declare function xosm_ag:bottomCountDistance($document as node()*,$street as node(), $k as xs:integer)
{
 let $list := xosm_ag:metricDistance($document,$street)
 return
...
declare function xosm_ag:topCountDistance($document as node()*, $street as node(), $k as xs:integer)
{
 let $list := xosm_ag:metricDistance($document,$street)
 return
 fn:subsequence(fn:sort($document,
